...
interface CacheInfo {
  ...
  summary?: { total_size: string; total_files: number; database_records: number }
}
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            管理应用程序的各种缓存数据，释放存储空间
            {cacheInfo.summary && (
              <span className="ml-2 text-sm">
                • 总计: {cacheInfo.summary.total_size} 
                • {cacheInfo.summary.total_files} 个文件
                • {cacheInfo.summary.database_records} 条记录
              </span>
            )}
          </p>
        </div>
